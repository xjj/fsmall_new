...
<div class="admin-wrap clearfix">
	...
	<hr class="line" />
	<form name="form" method="post" action="/<{$mod}>/<{$col}>/edit/<{$region_data.region_id}>">
	<input type="hidden" name="parent_id" value="<{$parent_id}>" />
	<input type="hidden" name="region_id" value="<{$region_data.region_id}>" />
	<table border="0" cellspacing="0" cellpadding="0" class="tbl2">
	  <tr>
		<td class="ltd" width="100">地区名称</td>
		<td><input type="text" name="region_name" size="30" class="txt" value="<{$region_data.region_name}>" /></td>
	  </tr>
	  <tr>
	  ...
	  <tr>
		...
		<td><input type="text" name="bigname" class="txt" size="10" value="<{$region_data.bigname}>" />
			<span class="lpad">市县级此项必填，用于快递分辨配送地址</span>
		</td>
	  </tr>
	  <tr>
		<td class="ltd">邮编</td>
		<td><input type="text" name="zipcode" class="txt" size="10" value="<{$region_data.zipcode}>" />
		</td>
	  </tr>
	  <tr>
		<td class="ltd">排序</td>
		<td><input type="text" name="displayorder" size="10" class="txt" value="<{$region_data.displayorder}>" />
		</td>
	  </tr>
	  <tr>
		<td class="ltd"></td>
