...
<div class="admin-wrap">
	<div class="col-box clearfix">
		...
		<a href="/<{$mod}>/<{$col}>/fee/<{$shipping_data.shipping_id}>" class="btn btn3 lpad">返回到（<{$shipping_data.shipping_name}>）配送费用列表</a>
	</div>
	
	<hr class="line" />
	<form name="form" method="post" action="/<{$mod}>/<{$col}>/fee/<{$shipping_data.shipping_id}>/add">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl2">
	  <tr>
		...
		<td><input type="text" name="fee_name" class="txt" size="30" /></td>
	  </tr>
	  <tr>
	  	<td class="ltd">首重费用</td>
		<td><input type="text" name="fee_base" value="0" class="txt" size="10" />
			<span class="lpad">一千克内费用</span>
		</td>
	  </tr>
	  <tr>
	  	<td class="ltd">续重费用</td>
		<td><input type="text" name="fee_step" value="0" class="txt" size="10" />
			<span class="lpad">超过一千克部分，每千克费用</span>
		</td>
	  </tr>
	  <tr>
	  	<td class="ltd">免费额度</td>
		<td><input type="text" name="free_amount" value="0" class="txt" size="10" />
	  ...
	  <tr>
		...
		<td>
			...
			<{foreach $province_items as $item}>
				<li><input type="checkbox" name="region_id[]" value="<{$item.region_id}>" /><span class="lpad"><{$item.region_name}></span></li>
			<{/foreach}>
			<{/if}>
			</ul>
		</td>
