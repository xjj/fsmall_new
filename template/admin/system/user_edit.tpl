...
<div class="admin-wrap">
	...
	<form name="form" method="post" action="/<{$mod}>/<{$col}>/edit/<{$user_data.uid}>">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl2">
	  <tr>
		...
		<td><input type="text" name="uname" class="txt" value="<{$user_data.uname}>" size="30" />
			<span class="lpad">2个汉字（4个字母） ~ 10个汉字（20个字母）</span>
		</td>
	  </tr>
	  <tr>
		<td class="ltd">邮箱</td>
		<td><input type="text" name="email" class="txt" value="<{$user_data.email}>" size="30" /></td>
	  </tr>
	  <tr>
