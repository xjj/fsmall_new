	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbl">
	  ...
	  <tr>
		...
		<td><{if $item.status eq 1}>
				<a href="/<{$mod}>/<{$col}>/category/hide/<{$item.id}>?page=<{$smarty.get.page}>" class="green">是</a>
			<{else}>
				<a href="/<{$mod}>/<{$col}>/category/show/<{$item.id}>?page=<{$smarty.get.page}>" class="red">否</a>
			<{/if}>
		</td>
		<td><{if $item.is_over eq 0}>
				<a href="javascript:;" data-href="/<{$mod}>/<{$col}>/category/over/<{$item.id}>?page=<{$smarty.get.page}>" class="btn-over">结束</a>
				<span class="dl">|</span>
				<a href="/<{$mod}>/<{$col}>/category/edit/<{$item.id}>">编辑</a>
				<span class="dl">|</span>
			<{/if}>
			<a href="javascript:;" class="btn-del" data-href="/<{$mod}>/<{$col}>/category/del/<{$item.id}>?page=<{$smarty.get.page}>">删除</a>
		</td>
	  </tr>
	  <{/foreach}>
	  <{/if}>
