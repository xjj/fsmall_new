 ...
 *	品牌折扣
 +-------------------------------
 */
class Discount_Brand extends Front {
	
	//信息
	function info($id){
		if (!isint($id) || $id <= 0){return false;}
		$sql = 'SELECT * FROM `product_discount_brand` WHERE id = '.$id;
		return $this -> db -> row($sql);
	}
	
	//获取折扣列表
	function items($page = 1, $pagesize = 20){
		$sql = 'SELECT * FROM `product_discount_brand` WHERE is_delete = 0 ORDER BY status DESC, id DESC LIMIT '.($page-1)*$pagesize.','.$pagesize;
		$items = $this -> db -> rows($sql);
		
		$sql = 'SELECT COUNT(*) AS num FROM `product_discount_brand` WHERE is_delete = 0';
		$row = $this -> db -> row($sql);
		$total = $row['total'];
	...
	function clear(){
		...
		$this -> db -> update('product_discount_brand', array('is_over' => 1), $where);
	}
	...
	function add($params){
		...
		return $this -> db -> insert('product_discount_brand', array(
			'cat_id' => $cat_id,
			'brand_id' => $brand_id,
			'discount' => $discount,
	...
	function update($id, $params){
		...
		return $this -> db -> update('product_discount_brand', array(
			'brand_id' => $brand_id,
			'cat_id' => $cat_id,
			'discount' => $discount,
			'start_time' => $start_line,
	...
	function delete($id){
		if (!isint($id) || $id <= 0){return false;}
		return $this -> db -> update('product_discount_brand', array('is_delete' => 1, 'delete_time' => time()), array('id' => $id));
	}
	...
	function validate($id, $status){
		...
		return $this -> db -> update('product_discount_brand', array('status' => $status), array('id' => $id));
	}
	
	//设置
	function over($id){
		if (!isint($id) || $id <= 0){return false;}
		return $this -> db -> update('product_discount_brand', array('is_over' => 1, 'over_time' => time()), array('id' => $id));
	}
} 