...
class Order_Product_Barcode extends Front {
	...
	//获取订单商品的物流信息 -- 数目统计
	function counter($order_id){
		$sql = 'SELECT op_id, op_status FROM `order_product_barcode` WHERE order_id = '.$order_id.'';
		$rows = $this -> db -> rows($sql);
		if ($rows){
			$data = array('user_cancel' => 0, 'kr_cancel' => 0, 'cn_cancel' => 0, 'no_send' => 0, 'kr_send' => 0, 'cn_recv' => 0, 'cn_send' => 0, 'user_recv' => 0);
			foreach ($rows as $row){
